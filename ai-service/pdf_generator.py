...
# ═════════════════════════════════════════════════════════════════════════════
# SECURITY TEST — PREMIUM REPORT (mirrors the Smoke Test public report style,
# WITHOUT any screenshot section)
#
# HOW TO INTEGRATE
# ----------------
# 1. Paste this whole block into pdf_generator.py (anywhere after the existing
#    imports and color palette — it reuses NAVY, GOLD, RED, section_header,
#    sub_section_header, stat_card, _make_score_gauge, _build_certificate_card,
#    _grade_from_score, PRIORITY_COLORS, _call_groq_for_plan, etc.).
# 2. It DOES NOT touch build_security_scenarios / build_security_category_summary /
#    build_security_results_table / _call_groq_security_recommendations —
#    those already exist in your file and are reused as-is.
# 3. Replace your current `_generate_security_pdf` function with the new one
#    defined at the bottom of this file (same name — Python will just use
#    the last definition, or delete the old one to avoid confusion).
# 4. No changes needed in security_generator.py, security_runner.py, or main.py.
# ═════════════════════════════════════════════════════════════════════════════

SECURITY_ACCENT = RED  # '#ef4444' — reuses the existing RED color already defined
...
SEC_CATEGORY_ORDER = ['Auth', 'XSS', 'Session', 'Navigation', 'Headers', 'Info Exposure']

SEC_CATEGORY_LABELS = {
    'auth': 'Auth', 'xss': 'XSS', 'session': 'Session',
    'navigation': 'Navigation', 'headers': 'Headers', 'info_exposure': 'Info Exposure',
}


def _security_category_label(test: dict) -> tuple:
    cat = (test.get('category') or 'auth').lower()
    label = SEC_CATEGORY_LABELS.get(cat, 'Auth')
    color = SEC_CAT_COLORS.get(cat, '#64748b')
    return label, color


def _security_chapter_header(elements, number: str, title: str):
    """Premium numbered section divider — same visual language as the smoke/k6 reports."""
    num_display = str(number).zfill(2)

    badge = Table([[Paragraph(
        f'<font color="white" size="13"><b>{num_display}</b></font>',
        ParagraphStyle('SecChapNum', fontSize=13, fontName='Helvetica-Bold',
                        alignment=TA_CENTER, leading=15))
    ]], colWidths=[14*mm], rowHeights=[14*mm])
    badge.setStyle(TableStyle([
        ('BACKGROUND', (0,0), (-1,-1), SECURITY_ACCENT),
        ('VALIGN',     (0,0), (-1,-1), 'MIDDLE'),
        ('ALIGN',      (0,0), (-1,-1), 'CENTER'),
    ]))
    ...
    title_block = Table([
        [Paragraph(f'<font color="#94a3b8" size="7"><b>SECTION {num_display}</b></font>',
                   ParagraphStyle('SecChapEyebrow', fontSize=7, fontName='Helvetica-Bold', leading=8.5))],
        [Paragraph(f'<font color="#1e293b" size="14"><b>{title}</b></font>',
                   ParagraphStyle('SecChapTitle', fontSize=14, fontName='Helvetica-Bold', leading=17))],
    ], colWidths=[148*mm])
    ...
    elements.append(Spacer(1, 10))
    elements.append(row)
    elements.append(HRFlowable(width='100%', thickness=2, color=SECURITY_ACCENT, spaceBefore=6, spaceAfter=12))


def _security_insight_box(text, color, width=161, label="AI Analysis"):
    tbl = Table([[Paragraph(
        f'<font color="{color}" size="7.5"><b>{label}: </b></font>'
        f'<font color="#475569" size="7.5">{text}</font>',
        ParagraphStyle('SecInsight', fontSize=7.5, fontName='Helvetica', leading=11))
    ]], colWidths=[width*mm])
    tbl.setStyle(TableStyle([
...
def _compute_security_score(tests: list) -> tuple:
    """Weighted security score — auth/xss/session checks (higher blast radius) count more
    than headers/info_exposure/navigation."""
    if not tests:
        return 0, 'Critical', '#ef4444'

    HIGH_CATS = {'auth', 'xss', 'session'}
    total_weight = 0
    earned_weight = 0
    for t in tests:
        cat = (t.get('category') or 'auth').lower()
        ...
        if status == 'pass':
            ...
            earned_weight += w * 0.5  # partial credit for warnings
...
def _make_security_category_chart(tests: list):
    """Passed vs Failed vs Warn by category — bar chart."""
    import matplotlib.pyplot as plt
    import numpy as np
    from io import BytesIO as BIO
    from reportlab.platypus import Image as RLImage

    cats = {c: {'pass': 0, 'fail': 0, 'warn': 0} for c in SEC_CATEGORY_ORDER}
    for t in tests:
        label, _ = _security_category_label(t)
        s = t.get('status', 'warn')
        if s in cats.get(label, {}):
            cats[label][s] += 1
        elif label in cats:
            if s == 'pass': cats[label]['pass'] += 1
            elif s == 'fail': cats[label]['fail'] += 1
            else: cats[label]['warn'] += 1

    labels = [c for c in SEC_CATEGORY_ORDER if sum(cats[c].values()) > 0] or SEC_CATEGORY_ORDER
    p = [cats[c]['pass'] for c in labels]
    w = [cats[c]['warn'] for c in labels]
    ...
    ax.bar(x, w, width=0.55, bottom=p, color='#f59e0b', edgecolor='white', label='Warn', zorder=3)
    bottom2 = [pi + wi for pi, wi in zip(p, w)]
    ax.bar(x, f, width=0.55, bottom=bottom2, color='#ef4444', edgecolor='white', label='Failed', zorder=3)
    ax.set_xticks(x)
...
def _make_security_status_bar_chart(tests: list):
    """Pass/Fail/Warn distribution — horizontal bar chart."""
    import matplotlib.pyplot as plt
    from io import BytesIO as BIO
    from reportlab.platypus import Image as RLImage
    ...
    pass_c = sum(1 for t in tests if t.get('status') == 'pass')
    fail_c = sum(1 for t in tests if t.get('status') == 'fail')
    warn_c = sum(1 for t in tests if t.get('status') == 'warn')
    total  = pass_c + fail_c + warn_c or 1

    labels = ['Passed', 'Failed', 'Warned']
    values = [pass_c, fail_c, warn_c]
    ...
    ax.invert_yaxis()
    fig.text(0.02, 0.95, 'Pass / Fail / Warn Distribution', fontsize=11, fontweight='bold', color='#1e293b', va='top')
...
# ─────────────────────────────────────────────────────────────────────────────
def build_security_overview_hero(elements, tests: list, ai_data: dict, score: int, score_label: str,
    ...
    CRITICAL_CATS = {'auth', 'xss', 'session'}
    critical_fail = any(
        t.get('status') == 'fail' and (t.get('category') or '').lower() in CRITICAL_CATS
        for t in tests
    )

    if critical_fail:
        overall_status, status_color = 'CRITICAL VULNERABILITIES', '#ef4444'
        risk_level, risk_color = 'HIGH', '#ef4444'
        deploy_text, deploy_color = 'NOT READY', '#ef4444'
        status_explain = (
            'One or more critical checks failed in Auth, XSS, or Session categories. These expose '
            'the application to real attack vectors — deployment is not recommended until resolved.')
    elif fail_count > 0 or warn_count > 0:
        overall_status, status_color = 'ISSUES DETECTED', '#f59e0b'
        risk_level, risk_color = 'MEDIUM', '#f59e0b'
        deploy_text, deploy_color = 'READY W/ CAUTION', '#f59e0b'
        status_explain = (
            f'No critical auth/session/xss failures, but {fail_count} check(s) failed and {warn_count} '
            f'raised warnings (headers, navigation, info exposure). Review before deploying with confidence.')
    else:
        ...
        status_explain = (
            'Every security check passed, including authentication, XSS, and session checks. '
            'No vulnerabilities or warnings detected on the frontend.')

    elements.append(section_header('', 'Security Test Overview', SECURITY_ACCENT))
    elements.append(Spacer(1, 4))
    elements.append(Paragraph(
        f'<font color="#64748b" size="7.5"><i>{status_explain}</i></font>',
        ParagraphStyle('SecStatusExplain', fontSize=7.5, fontName='Helvetica', leading=10)))
    elements.append(Spacer(1, 8))
    ...
    def _hero_stat(label, value, color):
        return Table([[Paragraph(
            ...
            ParagraphStyle('SecHeroStat', fontSize=10, fontName='Helvetica', leading=16,
                           alignment=TA_CENTER))
    ...
    row = Table([[
        ...
        _hero_stat('SECURITY SCORE', f'{score}/100', score_color),
    ...
    if not ai_summary_text:
        ...
        total = len(tests) or 1
        ai_summary_text = (
            f'This security audit executed {total} checks against <b>{url}</b>, with {pass_count} passed, '
            f'{fail_count} failed, and {warn_count} warned ({pass_rate}% pass rate). '
            + ('Critical vulnerabilities were detected and must be resolved before deployment.' if critical_fail
               else 'No critical vulnerabilities were detected in this run.')
        )
    elements.append(_security_insight_box(ai_summary_text, '#4f46e5', label='AI Summary'))
    elements.append(Spacer(1, 16))
...
# ─────────────────────────────────────────────────────────────────────────────
def build_security_key_metrics_cards(elements, generation_data: dict, tests: list):
    elements.append(section_header('', 'Key Metrics', SECURITY_ACCENT))
    elements.append(Spacer(1, 4))
    elements.append(Paragraph(
        '<font color="#64748b" size="7.5"><i>'
        'Snapshot of this run\'s execution footprint — how long it took, how many checks fell into '
        'each category, and where critical checks are concentrated.'
        '</i></font>',
        ParagraphStyle('SecKMInfo', fontSize=7.5, fontName='Helvetica', leading=10)))
    elements.append(Spacer(1, 8))
    ...
    critical_count = sum(
        1 for t in tests
        if t.get('status') == 'fail' and (t.get('category') or '').lower() in CRITICAL_CATS
    )

    cat_counts = {'Auth': 0, 'XSS': 0, 'Headers': 0}
    for t in tests:
        label, _ = _security_category_label(t)
    ...
    metrics = [
        ...
        ('AUTH CHECKS',      str(cat_counts['Auth']),                        '#6366f1'),
        ('XSS CHECKS',       str(cat_counts['XSS']),                         '#ef4444'),
        ('HEADER CHECKS',    str(cat_counts['Headers']),                     '#3b82f6'),
    ...
    def _metric_card(label, value, color):
        cell = Table([[Paragraph(
            ...
            ParagraphStyle('SecKeyMetricC', fontSize=9, fontName='Helvetica', leading=15,
                           alignment=TA_CENTER))
        ]], colWidths=[54*mm])
    ...
    for i in range(0, len(metrics), 3):
        ...
        while len(row_cells) < 3:
            row_cells.append(Paragraph('', ParagraphStyle('SecKeyMetricEmpty')))
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION — Security Score Hero (gauge)
# ─────────────────────────────────────────────────────────────────────────────
def build_security_score_hero(elements, score: int, score_label: str, score_color: str):
    elements.append(section_header('', 'Security Score', HexColor(score_color)))
    elements.append(Spacer(1, 10))
    try:
    ...
    right_col = [
        Paragraph(f'<font color="{score_color}" size="15"><b>{score_label}</b></font>',
                  ParagraphStyle('SecHeroLabel', fontSize=15, fontName='Helvetica-Bold', leading=18)),
        Spacer(1, 6),
        Paragraph(
            '<font color="#475569" size="8.5">Security Score weighs high-blast-radius checks '
            '(Authentication, XSS, Session management) more heavily than lower-impact checks '
            '(Headers, Navigation, Info Exposure). Excellent ≥ 90 · Good ≥ 75 · Acceptable ≥ 50 · Critical below.</font>',
            ParagraphStyle('SecHeroDesc', fontSize=8.5, fontName='Helvetica', leading=13)),
    ]
    if gauge_img:
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION — Methodology (wraps the existing build_security_scenarios)
# ─────────────────────────────────────────────────────────────────────────────
def build_security_methodology(elements, tests: list, url: str):
    elements.append(section_header('', 'Security Test Methodology', SECURITY_ACCENT))
    elements.append(Spacer(1, 6))
    elements.append(Paragraph(
        'This security audit validates frontend attack surface exposure — unauthenticated access to '
        'protected routes, XSS input sanitization, session/token handling, direct navigation bypass, '
        'security headers, and information exposure in the DOM/localStorage. Tests run automatically '
        'using Playwright, authenticated via a stored JWT token where applicable.',
        ParagraphStyle('SecMethoTxt', fontSize=8.5, fontName='Helvetica', leading=13,
                       textColor=HexColor('#475569'))))
    elements.append(Spacer(1, 16))
    build_security_scenarios(elements, tests, url)  # existing function — reused as-is


# ─────────────────────────────────────────────────────────────────────────────
# SECTION — Execution Timeline
# ─────────────────────────────────────────────────────────────────────────────
def build_security_execution_timeline(elements, generation_data: dict, tests: list):
    elements.append(section_header('', 'Execution Timeline', SECURITY_ACCENT))
    elements.append(Spacer(1, 4))
    elements.append(Paragraph(
        '<font color="#64748b" size="7.5"><i>'
        'Sequential phases of this security test run, from token acquisition to report generation.'
        '</i></font>', ParagraphStyle('SecTLInfo', fontSize=7.5, fontName='Helvetica', leading=10)))
    elements.append(Spacer(1, 8))

    def _has_cat(catkey):
        return any((t.get('category') or '').lower() == catkey for t in tests)

    steps = [
        ('Browser Started',     'Chromium (headless), JWT injected', True),
        ('Auth Checks',         'Unauthenticated / invalid token access', _has_cat('auth')),
        ('XSS Checks',          'Script injection into input fields', _has_cat('xss')),
        ('Session Checks',      'localStorage inspection, logout verification', _has_cat('session')),
        ('Navigation Checks',   'Direct URL access without login', _has_cat('navigation')),
        ('Header Checks',       'Security response headers verification', _has_cat('headers')),
        ('Info Exposure Checks','DOM scan for passwords/secrets/API keys', _has_cat('info_exposure')),
    ...
    for i, (title, detail, executed) in enumerate(steps):
        ...
        num_badge = Table([[Paragraph(
            ...
            ParagraphStyle('SecTLNum', fontSize=8, fontName='Helvetica-Bold', alignment=TA_CENTER))
        ...
        text_cell = Paragraph(
            ...
            ParagraphStyle('SecTLText', fontSize=8.5, fontName='Helvetica', leading=11))
...
# ─────────────────────────────────────────────────────────────────────────────
def build_security_top_findings(elements, tests: list):
    elements.append(section_header('', 'Top Findings', SECURITY_ACCENT))
    elements.append(Spacer(1, 4))
    elements.append(Paragraph(
        '<font color="#64748b" size="7.5"><i>'
        'Quick-glance summary of the most relevant outcomes from this run.'
        '</i></font>', ParagraphStyle('SecTFInfo', fontSize=7.5, fontName='Helvetica', leading=10)))
    elements.append(Spacer(1, 8))
    ...
    for t in tests:
        ...
        if status == 'pass':
            ...
            warns.append(name + (f' — {reason[:60]}' if reason else ''))

    display_list = ([(False, f_) for f_ in fails[:6]] +
                     [('warn', w_) for w_ in warns[:4]] +
                     [(True, p_) for p_ in passes[:6]])
    if not display_list:
        elements.append(Paragraph('<font color="#94a3b8" size="8">No findings to display.</font>',
                                   ParagraphStyle('SecTFNone', fontSize=8, fontName='Helvetica')))
        elements.append(Spacer(1, 16))
        return
    ...
    for ok, text in display_list:
        ...
        rows.append([Paragraph(
            ...
            ParagraphStyle('SecTFItem', fontSize=8, fontName='Helvetica', leading=13))])
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION — AI Analysis (narrative)
# ─────────────────────────────────────────────────────────────────────────────
def build_security_ai_analysis(elements, ai_data: dict, tests: list, score: int, pass_rate: int, url: str):
    ...
    if not ai_summary_text:
        ...
        fail_count = sum(1 for t in tests if t.get('status') == 'fail')
        ai_summary_text = (
            f'This security audit validated {total} checks on <b>{url}</b>, with {pass_count} passed '
            f'and {fail_count} failed. Security testing confirms whether authentication, session handling, '
            f'and input sanitization are correctly enforced before deeper testing proceeds.'
        )
    elements.append(Paragraph(f'<font color="#475569" size="8.5">{ai_summary_text}</font>',
                               ParagraphStyle('SecAIIntro', fontSize=8.5, fontName='Helvetica', leading=13)))
    elements.append(Spacer(1, 12))
    ...
    for title, catkey, color in subsection_defs:
        related = [t for t in tests if (t.get('category') or '').lower() == catkey]
        total_n = len(related)
        if total_n == 0:
            continue
        fail_n = sum(1 for t in related if t.get('status') == 'fail')
        warn_n = sum(1 for t in related if t.get('status') == 'warn')
        if fail_n == 0 and warn_n == 0:
            text = f'All {total_n} check(s) passed — this area is fully secured with no issues detected.'
        else:
            issues = [t.get('name', '') for t in related if t.get('status') in ('fail', 'warn')][:3]
            text = f'{fail_n + warn_n} of {total_n} check(s) flagged in this area: {", ".join(issues)}. Investigate before deployment.'
        elements.append(sub_section_header(title, color))
        elements.append(Spacer(1, 4))
        elements.append(_security_insight_box(text, color, label="Summary"))
        elements.append(Spacer(1, 10))

    CRITICAL_CATS = {'auth', 'xss', 'session'}
    critical_fail = any(t.get('status') == 'fail' and (t.get('category') or '').lower() in CRITICAL_CATS for t in tests)
    fail_count = sum(1 for t in tests if t.get('status') == 'fail')
    warn_count = sum(1 for t in tests if t.get('status') == 'warn')

    if critical_fail:
        depl_text = 'Deployment NOT recommended — critical auth/XSS/session vulnerabilities detected.'
        depl_color = '#ef4444'
    elif fail_count > 0 or warn_count > 0:
        depl_text = f'Deployment possible with caution — {fail_count} failed / {warn_count} warned check(s). Review before promoting to production.'
        depl_color = '#f59e0b'
    else:
        depl_text = 'Deployment ready — all security checks passed, no vulnerabilities detected.'
        depl_color = '#10b981'
    elements.append(sub_section_header('Deployment Readiness', depl_color))
    elements.append(Spacer(1, 4))
    elements.append(_security_insight_box(depl_text, depl_color, label="Summary"))
    elements.append(Spacer(1, 16))


# ─────────────────────────────────────────────────────────────────────────────
# SECTION — AI Recommendations (table only)
# ─────────────────────────────────────────────────────────────────────────────
def build_security_ai_recommendations_table(elements, ai_data: dict):
    ...
    elements.append(Paragraph(
        '<font color="#64748b" size="7.5"><i>'
        'Consolidated recommendations derived from execution evidence and vulnerability analysis.'
        '</i></font>', ParagraphStyle('SecRSInfo2', fontSize=7.5, fontName='Helvetica', leading=10)))
    elements.append(Spacer(1, 8))

    if recs:
        hdr = [
            Paragraph('<font color="#ffffff"><b>Priority</b></font>', ParagraphStyle('SecRH1b', fontSize=8, fontName='Helvetica-Bold', alignment=TA_CENTER)),
            Paragraph('<font color="#ffffff"><b>Category</b></font>', ParagraphStyle('SecRH2b', fontSize=8, fontName='Helvetica-Bold')),
            Paragraph('<font color="#ffffff"><b>Issue</b></font>', ParagraphStyle('SecRH3b', fontSize=8, fontName='Helvetica-Bold')),
            Paragraph('<font color="#ffffff"><b>Recommended Fix</b></font>', ParagraphStyle('SecRH4b', fontSize=8, fontName='Helvetica-Bold')),
        ]
        rows = [hdr]
        row_styles = []
        for i, rec in enumerate(recs):
            ...
            rows.append([
                Paragraph(f'<font color="{pc}"><b>{priority.upper()}</b></font>', ParagraphStyle('SecRPb', fontSize=7.5, fontName='Helvetica-Bold', alignment=TA_CENTER)),
                Paragraph(f'<font color="#4f46e5" size="7.5"><b>{(rec.get("category","") or "").upper()}</b></font>', ParagraphStyle('SecRCb', fontSize=7.5, fontName='Helvetica-Bold')),
                Paragraph(f'<font color="#1e293b" size="7.5">{rec.get("issue","")[:70]}</font>', ParagraphStyle('SecRIb', fontSize=7.5, fontName='Helvetica', leading=10)),
                Paragraph(f'<font color="#475569" size="7.5">{rec.get("fix","")[:80]}</font>', ParagraphStyle('SecRFb', fontSize=7.5, fontName='Helvetica', leading=10)),
            ])
            if i % 2 == 1:
                row_styles.append(('BACKGROUND', (0, i+1), (-1, i+1), LIGHT_BG))
        ...
        elements.append(Paragraph(
            ...
            ParagraphStyle('SecNoRecb', fontSize=8, fontName='Helvetica')))
    elements.append(Spacer(1, 16))
...
# ─────────────────────────────────────────────────────────────────────────────
def build_security_executive_summary(elements, ai_data: dict, score: int, url: str):
    recs = (ai_data or {}).get('recommendations', []) or []
    if not recs:
        return
    ...
    elements.append(Paragraph('<font color="#1e293b" size="9"><b>Top Priority Actions</b></font>',
                               ParagraphStyle('SecExecH', fontSize=9, fontName='Helvetica-Bold')))
    elements.append(Spacer(1, 8))
    for i, item in enumerate(top):
        card = Table([[Paragraph(
            ...
            ParagraphStyle('SecExecCard', fontSize=8.5, fontName='Helvetica', leading=12))
        ]], colWidths=[168*mm])
    ...
    insight = (
        f'These {len(top)} action(s) are proactive hardening measures — the current score of {score}/100 '
        f'for {url} already reflects a solid security baseline.' if score >= 90 else
        f'Addressing these {len(top)} action(s) targets the largest contributors to the current score '
        f'of {score}/100 for {url}. Re-run the security suite after applying them to confirm improvement.'
    )
    elements.append(_security_insight_box(insight, '#c9a227'))
    elements.append(Spacer(1, 8))


# ─────────────────────────────────────────────────────────────────────────────
# SECTION — Environment & Execution Info
# ─────────────────────────────────────────────────────────────────────────────
def build_security_environment_info(elements, generation_data: dict, tests: list):
    total_ms = 0
    try:
        for t in tests:
    ...
    items = [
        ...
        ('AUTH METHOD',     'JWT (stored token)',                                  '#f59e0b'),
    ...
    def _env_card(label, value, color):
        cell = Table([[Paragraph(
            ...
            ParagraphStyle('SecEnvC', fontSize=9, fontName='Helvetica', leading=15,
                           alignment=TA_CENTER))
        ]], colWidths=[54*mm])
        cell.setStyle(TableStyle([
            ('BACKGROUND',    (0,0), (-1,-1), WHITE),
    ...
    for i in range(0, len(items), 3):
        ...
        while len(row_cells) < 3:
            row_cells.append(Paragraph('', ParagraphStyle('SecEnvEmpty')))
        card_rows.append(row_cells)
    ...
    elements.append(KeepTogether([
        section_header('', 'Environment & Execution Info', SECURITY_ACCENT),
        Spacer(1, 6),
        Paragraph(
            '<font color="#64748b" size="7.5"><i>'
            'Browser, auth method, and framework used to run this security audit — '
            'for reproducibility of the results above.'
            '</i></font>',
            ParagraphStyle('SecEnvInfo', fontSize=7.5, fontName='Helvetica', leading=10)),
        Spacer(1, 6),
        outer,
    ]))
    elements.append(Spacer(1, 16))
# ═════════════════════════════════════════════════════════════════════════════
# PATCH — Vulnerability Severity Distribution + Vulnerability Summary
# Colle ce bloc juste AVANT le commentaire :
#   # MAIN — _generate_security_pdf (REPLACES the existing one)
# ═════════════════════════════════════════════════════════════════════════════
...
SEC_SEVERITY_ORDER = ['Critical', 'High', 'Medium', 'Low', 'Informational']


def _security_severity_counts(tests: list) -> dict:
    """Counts detected issues (failed checks) by severity level, dynamically from results."""
    counts = {s: 0 for s in SEC_SEVERITY_ORDER}
    for t in tests:
...
def _make_security_severity_chart(tests: list):
    """Donut chart — Vulnerability Severity Distribution (replaces Execution Timeline)."""
    import matplotlib.pyplot as plt
    import matplotlib.patches as mpatches
    from io import BytesIO as BIO
    from reportlab.platypus import Image as RLImage

    counts = _security_severity_counts(tests)
    labels = [l for l in SEC_SEVERITY_ORDER if counts[l] > 0]
    sizes  = [counts[l] for l in labels]
    colors = [SEC_SEVERITY_COLORS[l.lower()] for l in labels]
    total  = sum(sizes)
    ...
    if total == 0:
        ax.pie([1], colors=['#e2e8f0'], startangle=90,
               wedgeprops={'edgecolor': 'white', 'linewidth': 3, 'width': 0.55})
        ax.text(0, 0.05, '0', ha='center', va='center',
                 fontsize=22, fontweight='bold', color='#94a3b8')
        ax.text(0, -0.2, 'Issues Found', ha='center', va='center',
                 fontsize=8, color='#64748b', fontweight='bold')
    else:
        ...
        ax.text(0, 0.08, f'{total}', ha='center', va='center',
                 fontsize=22, fontweight='bold', color=worst_color)
        ax.text(0, -0.22, 'Issues Found', ha='center', va='center',
                 fontsize=8, color='#64748b', fontweight='bold')
        legend_patches = [mpatches.Patch(color=c, label=f'{l}  ({v})')
                           for c, l, v in zip(colors, labels, sizes)]
        ax.legend(handles=legend_patches, loc='lower center', bbox_to_anchor=(0.5, -0.16),
                  ncol=min(len(labels), 3), fontsize=7.5, frameon=False,
                  handlelength=1.2, handleheight=0.8)

    fig.text(0.5, 0.97, 'Vulnerability Severity Distribution', fontsize=10.5, fontweight='bold',
              color='#1e293b', va='top', ha='center')
    plt.subplots_adjust(top=0.86, bottom=0.2)
    buf = BIO()
    fig.savefig(buf, format='png', dpi=180, bbox_inches='tight', facecolor='white', edgecolor='none')
    plt.close()
    buf.seek(0)
    return RLImage(buf, width=95*mm, height=95*mm), counts


def build_security_vulnerability_summary_table(elements, tests: list):
    """NEW SECTION — small severity summary table, placed right after the Security Score section."""
    counts = _security_severity_counts(tests)

    elements.append(section_header('', 'Vulnerability Summary', SECURITY_ACCENT))
    elements.append(Spacer(1, 6))

    hdr = [
        Paragraph('<font color="#ffffff"><b>Severity</b></font>',
                  ParagraphStyle('VSH1', fontSize=8, fontName='Helvetica-Bold')),
        Paragraph('<font color="#ffffff"><b>Count</b></font>',
                  ParagraphStyle('VSH2', fontSize=8, fontName='Helvetica-Bold', alignment=TA_RIGHT)),
    ]
    rows = [hdr]
    row_styles = []
    for i, sev in enumerate(SEC_SEVERITY_ORDER):
        color = SEC_SEVERITY_COLORS[sev.lower()]
        count = counts[sev]
        rows.append([
            Paragraph(f'<font color="{color}"><b>{sev}</b></font>',
                      ParagraphStyle(f'VSL{i}', fontSize=8.5, fontName='Helvetica-Bold')),
            Paragraph(f'<font color="{color}"><b>{count}</b></font>',
                      ParagraphStyle(f'VSV{i}', fontSize=8.5, fontName='Helvetica-Bold', alignment=TA_RIGHT)),
        ])
        if count > 0:
            row_styles.append(('BACKGROUND', (0, i+1), (-1, i+1), LIGHT_BG))

    tbl = Table(rows, colWidths=[100*mm, 68*mm], repeatRows=1)
    tbl.setStyle(TableStyle([
        ('BACKGROUND',    (0,0), (-1,0), NAVY),
        ('PADDING',       (0,0), (-1,-1), 7),
        ('LINEBELOW',     (0,0), (-1,-1), 0.4, BORDER),
        ('BOX',           (0,0), (-1,-1), 0.8, SECURITY_ACCENT),
        ('VALIGN',        (0,0), (-1,-1), 'MIDDLE'),
    ] + row_styles))
    elements.append(tbl)
    elements.append(Spacer(1, 16))
...
def _generate_security_pdf(generation_data: dict, tests: list) -> bytes:
    ...
    url       = generation_data.get('url', '')
    framework = generation_data.get('framework', 'Playwright')
    ai_data   = generation_data.get('_groq_recs', {}) or {}
    ...
    score, score_label, score_color = _compute_security_score(tests)
    ...
    def on_page_sec(canvas, doc):
        ...
        canvas.setFillColor(NAVY)
        canvas.rect(0, H - 52*mm, W, 52*mm, fill=1, stroke=0)
        canvas.setFillColor(SECURITY_ACCENT)
        canvas.rect(0, H - 54*mm, W, 2*mm, fill=1, stroke=0)
        canvas.setFillColor(SECURITY_ACCENT)
        canvas.rect(0, 0, 3, H - 54*mm, fill=1, stroke=0)
        canvas.setFillColor(LIGHT_BG)
    ...
    header_data = [[
        Paragraph('<font color="#ef4444"><b>NEX</b></font><font color="#ffffff">TEST</font>',
                  ParagraphStyle('SecLogo', fontSize=24, fontName='Helvetica-Bold')),
        Paragraph(f'<font color="#64748b">Generated</font><br/>'
                  f'<font color="#94a3b8">{datetime.now().strftime("%B %d, %Y  •  %H:%M")}</font>',
                  ParagraphStyle('SecDate', fontSize=8.5, fontName='Helvetica', alignment=TA_RIGHT, leading=13)),
    ]]
    ...
    elements.append(Paragraph('Security Test Report',
                               ParagraphStyle('SecTitle', fontSize=22, textColor=WHITE,
    ...
    info_tbl = Table([
        [Paragraph('<font color="#64748b">URL</font>', ParagraphStyle('SecIL1', fontSize=8, fontName='Helvetica-Bold', leading=12)),
         Paragraph(f'<font color="#1e293b">{url}</font>', ParagraphStyle('SecIV1', fontSize=8.5, fontName='Helvetica', leading=12))],
        [Paragraph('<font color="#64748b">Framework</font>', ParagraphStyle('SecIL2', fontSize=8, fontName='Helvetica-Bold', leading=12)),
         Paragraph(f'<font color="#ef4444"><b>{framework}</b></font>', ParagraphStyle('SecIV2', fontSize=8.5, fontName='Helvetica', leading=12))],
        [Paragraph('<font color="#64748b">Test Type</font>', ParagraphStyle('SecIL3', fontSize=8, fontName='Helvetica-Bold', leading=12)),
         Paragraph('<font color="#ef4444"><b>Security Test</b></font>', ParagraphStyle('SecIV3', fontSize=8.5, fontName='Helvetica', leading=12))],
        [Paragraph('<font color="#64748b">Execution Time</font>', ParagraphStyle('SecIL5', fontSize=8, fontName='Helvetica-Bold', leading=12)),
         Paragraph(f'<font color="#1e293b">{exec_time_disp}</font>', ParagraphStyle('SecIV5', fontSize=8.5, fontName='Helvetica', leading=12))],
        [Paragraph('<font color="#64748b">Generated</font>', ParagraphStyle('SecIL4', fontSize=8, fontName='Helvetica-Bold', leading=12)),
         Paragraph(f'<font color="#1e293b">{datetime.now().strftime("%Y-%m-%d  %H:%M")}</font>', ParagraphStyle('SecIV4', fontSize=8.5, fontName='Helvetica', leading=12))],
    ...
    # KPI cards
    elements.append(section_header('', 'Test Summary', SECURITY_ACCENT))
    elements.append(Spacer(1, 8))
    stats_data = [[
        stat_card(pass_count, 'PASSED', '#10b981', GREEN_BG),
        stat_card(fail_count, 'FAILED', '#ef4444', RED_BG),
        stat_card(warn_count, 'WARNED', '#f59e0b', ORANGE_BG),
    ...
    elements.append(Paragraph(
        ...
        'The Security Score further below is severity-weighted — Auth/XSS/Session checks count more '
        'than Headers/Navigation/Info Exposure — which is why the two numbers can differ.'
        '</i></font>', ParagraphStyle('SecRateNote2', fontSize=7, fontName='Helvetica', leading=9)))
    elements.append(Spacer(1, 20))

    build_security_overview_hero(elements, tests, ai_data, score, score_label, score_color, pass_rate, url)

    _security_chapter_header(elements, '1', 'Score & Coverage')

    build_security_key_metrics_cards(elements, generation_data, tests)
    build_security_score_hero(elements, score, score_label, score_color)
    build_security_vulnerability_summary_table(elements, tests)

    _security_chapter_header(elements, '2', 'Test Coverage')

    build_security_methodology(elements, tests, url)
    build_security_category_summary(elements, tests)  # existing function

    _security_chapter_header(elements, '3', 'Charts & Detailed Results')

    try:
        chart_img, cats_data = _make_security_category_chart(tests)
        cat_desc = (
            'This chart compares passed, warned, and failed checks across each security category, '
            'helping you quickly spot which areas need attention.'
        )
        worst_cat = max(cats_data, key=lambda c: cats_data[c]['fail'] + cats_data[c]['warn']) if cats_data else None
        if worst_cat and (cats_data[worst_cat]['fail'] + cats_data[worst_cat]['warn']) > 0:
            cat_insight = (
                f'{worst_cat} currently has the most issues '
                f'({cats_data[worst_cat]["fail"]} failed, {cats_data[worst_cat]["warn"]} warned) — '
                f'this is the category to prioritize first.'
            )
        else:
            cat_insight = 'No category shows any failures or warnings — coverage is clean across the board.'

        framed = Table([[chart_img]], colWidths=[161*mm])
        framed.setStyle(TableStyle([
            ('BOX', (0,0), (-1,-1), 1, SECURITY_ACCENT),
            ('BACKGROUND', (0,0), (-1,-1), WHITE),
            ('ALIGN', (0,0), (-1,-1), 'CENTER'),
            ('TOPPADDING', (0,0), (-1,-1), 8),
            ('BOTTOMPADDING', (0,0), (-1,-1), 8),
        ]))
        elements.append(section_header('', 'Category Breakdown Chart', SECURITY_ACCENT))
        elements.append(Spacer(1, 4))
        elements.append(Paragraph(
            f'<font color="#64748b" size="7.5"><i>{cat_desc}</i></font>',
            ParagraphStyle('SecCatDesc', fontSize=7.5, fontName='Helvetica', leading=10)))
        elements.append(Spacer(1, 8))
        elements.append(framed)
        elements.append(Spacer(1, 6))
        elements.append(_security_insight_box(cat_insight, SECURITY_ACCENT.hexval() if hasattr(SECURITY_ACCENT, 'hexval') else '#ef4444'))
        elements.append(Spacer(1, 16))
    except Exception as e:
        print(f"[SECURITY PDF] category chart error: {e}")

    try:
        dist_img = _make_security_status_bar_chart(tests)
        dist_desc = (
            'This chart shows the overall distribution of test outcomes — how many checks '
            'passed, failed, or raised a warning during this security run.'
        )
        dist_insight = (
            f'Out of {total} executed checks: {pass_count} passed ({round(pass_count/total*100)}%), '
            f'{fail_count} failed ({round(fail_count/total*100)}%), and {warn_count} warned '
            f'({round(warn_count/total*100)}%).'
        )
        ...
        elements.append(section_header('', 'Pass / Fail / Warn Distribution', SECURITY_ACCENT))
        elements.append(Spacer(1, 4))
        elements.append(Paragraph(
            f'<font color="#64748b" size="7.5"><i>{dist_desc}</i></font>',
            ParagraphStyle('SecDistDesc', fontSize=7.5, fontName='Helvetica', leading=10)))
        elements.append(Spacer(1, 8))
        elements.append(framed_dist)
        elements.append(Spacer(1, 6))
        elements.append(_security_insight_box(dist_insight, '#0EA5E9'))
        elements.append(Spacer(1, 16))
    except Exception as e:
        print(f"[SECURITY PDF] status chart error: {e}")

    # Detailed Results — existing function, reused as-is
    build_security_results_table(elements, tests)

    build_security_top_findings(elements, tests)

    _security_chapter_header(elements, '4', 'AI Insights & Recommendations')

    build_security_ai_analysis(elements, ai_data, tests, score, pass_rate, url)
    build_security_ai_recommendations_table(elements, ai_data)

    action_plan = _call_groq_for_plan(tests, url)
    if action_plan:
        build_regression_action_plan(elements, action_plan)  # existing generic function, reused

    build_security_executive_summary(elements, ai_data, score, url)

    _security_chapter_header(elements, '5', 'Execution Context & Verdict')

    build_security_environment_info(elements, generation_data, tests)

    # ── FINAL VERDICT ────────────────────────────────────────────────────
    elements.append(section_header('', 'Final AI Verdict', SECURITY_ACCENT))
    elements.append(Spacer(1, 6))
    ...
    if critical_fail:
        ...
        risk_level = 'HIGH'
        vt = (f'Security Test FAILED — critical vulnerabilities detected on {url} (Auth/XSS/Session). '
              f'These issues must be fixed before production deployment.')
    elif fail_count > 0 or warn_count > 0:
        vc, vb, vbrd, vi = '#b45309', HexColor('#fffbeb'), ORANGE, '🟡'
        risk_level = 'MEDIUM'
        vt = (f'Security Test passed with {fail_count} failed / {warn_count} warned check(s) on {url}. '
              f'No critical auth/XSS/session issues, but the flagged checks should be reviewed before deployment.')
    else:
        vc, vb, vbrd, vi = '#059669', HexColor('#f0fdf4'), GREEN, '🟢'
        risk_level = 'LOW'
        vt = (f'Security Test PASSED — all {pass_count} security checks passed successfully on {url}. '
              f'No critical vulnerabilities detected on the frontend.')
        
    # Overall Health must reflect actual findings, not just the numeric score
    if critical_fail:
        health_status = 'High Risk'
    elif fail_count > 0 or warn_count > 0:
        health_status = 'Requires Remediation'
    else:
        health_status = score_label

    final_tbl = Table([[Paragraph(
        f'<font color="{vc}" size="9"><b>{vi}  Final Security Verdict</b></font><br/>'
        f'<font color="{vc}" size="8">{vt}</font><br/><br/>'
        f'<font color="#64748b" size="8"><b>Security Score: </b></font>'
        ...
        f'<font color="{vc}" size="8"><b>{health_status}</b></font>',
        ParagraphStyle('SecFV2', fontSize=8, fontName='Helvetica', leading=13))
    ]], colWidths=[168*mm])
    final_tbl.setStyle(TableStyle([
        ('BACKGROUND',    (0,0), (-1,-1), vb),
        ('BOX',           (0,0), (-1,-1), 2, vbrd),
    ...
    # ── CERTIFICATE ──────────────────────────────────────────────────────
    elements.append(section_header('', 'Certificate of Security Validation', SECURITY_ACCENT))
    elements.append(Spacer(1, 4))
    elements.append(Paragraph(
        '<font color="#64748b" size="7.5"><i>'
        'Official validation summary confirming the outcome of this security test run — '
        'issued automatically by NexTest AI based on the results above.'
        '</i></font>',
        ParagraphStyle('SecCertSectionInfo', fontSize=7.5, fontName='Helvetica', leading=10)))
    elements.append(Spacer(1, 8))
    sec_cert_data = {
        'global_score': score,
    ...
    elements.append(_build_certificate_card(sec_cert_data, url, title="CERTIFICATE OF SECURITY VALIDATION"))
...
# ─────────────────────────────────────────────────────────────────────────────
# SECURITY TEST — XLSX (mirrors _generate_security_pdf sections)
# ─────────────────────────────────────────────────────────────────────────────
def generate_security_xlsx(generation_data: dict, tests: list) -> bytes:
    url       = generation_data.get('url', '')
    framework = generation_data.get('framework', 'Playwright')
    ai_data   = generation_data.get('_groq_recs', {}) or {}
    ...
    score, score_label, score_color_hex = _compute_security_score(tests)
    grade, _ = _grade_from_score(score)

    HIGH_CATS = {'auth', 'xss', 'session'}
    critical_fail = any(
        t.get('status') == 'fail' and (t.get('category') or '').lower() in HIGH_CATS
        for t in tests
    )
    ...
    for r_ in range(1, 4):
        for c_ in range(1, 9):
            ws.cell(row=r_, column=c_).fill = PatternFill("solid", fgColor="0A0F1E")
    ws["A1"] = "NEXTEST — Security Test Report"
    ws["A1"].font = Font(name=_XLSX_FONT, bold=True, size=20, color="EF4444")
    ws["A1"].alignment = Alignment(horizontal="left", vertical="center", indent=1)
    ...
    info_rows = [
        ...
        ("Security Score", f"{score}/100 — {score_label}"),
        ("Generated", datetime.now().strftime("%Y-%m-%d %H:%M")),
    ]
    r = 6
    for lbl, val in info_rows:
        ...
        ws.merge_cells(start_row=r, start_column=2, end_row=r, end_column=6)
        vc = ws.cell(row=r, column=2, value=val)
        vc.font = Font(name=_XLSX_FONT, size=10, color="1E293B", bold=(lbl == "Security Score"))
        for col in range(1, 7):
            ws.cell(row=r, column=col).border = _XLSX_BORDER
        r += 1
    ...
    stats = [
        ("PASSED", pass_count, "10B981", "D1FAE5"),
        ("FAILED", fail_count, "EF4444", "FEE2E2"),
        ("WARNED", warn_count, "F59E0B", "FEF3C7"),
        ("PASS RATE", f"{pass_rate}%", rate_color,
         "D1FAE5" if pass_rate >= 80 else "FEF3C7" if pass_rate >= 50 else "FEE2E2"),
        ("SCORE", score, score_color_hex.lstrip('#').upper(),
         "D1FAE5" if score >= 80 else "FEF3C7" if score >= 50 else "FEE2E2"),
    ...
    overall_status = 'CRITICAL VULNERABILITIES' if critical_fail else ('ISSUES DETECTED' if (fail_count > 0 or warn_count > 0) else 'ALL CHECKS PASSED')
    risk_level = 'HIGH' if critical_fail else ('MEDIUM' if (fail_count > 0 or warn_count > 0) else 'LOW')
    deploy_text = 'NOT READY' if critical_fail else ('READY W/ CAUTION' if (fail_count > 0 or warn_count > 0) else 'READY')
    status_color = "EF4444" if critical_fail else ("F59E0B" if (fail_count > 0 or warn_count > 0) else "10B981")

    ws.cell(row=r, column=1, value="Security Test Overview").font = Font(
        name=_XLSX_FONT, bold=True, size=12, color="1E293B")
    r += 1
    hero_items = [
        ("OVERALL STATUS", overall_status, status_color),
        ("SECURITY SCORE", f"{score}/100", score_color_hex.lstrip('#').upper()),
        ("RISK LEVEL", risk_level, status_color),
        ("DEPLOYMENT", deploy_text, status_color),
    ]
    col = 1
    ...
    CAT_COLORS = {'auth':'6366F1','xss':'EF4444','session':'F59E0B','navigation':'10B981','headers':'3B82F6','info_exposure':'8B5CF6'}
    cats = {}
    for t in tests:
        c = (t.get('category') or 'auth').lower()
        if c not in cats:
    ...
    cat_hdr_row = r
    _perf_xlsx_header_row(ws, r, ["Category", "Total", "Passed", "Failed", "Warn"])
    r += 1
    for cat, d in cats.items():
        c1 = ws.cell(row=r, column=1, value=cat.upper())
        c1.font = Font(name=_XLSX_FONT, bold=True, size=9, color=CAT_COLORS.get(cat, "64748B"))
        c1.border = _XLSX_BORDER
        for i, v in enumerate([d['total'], d['pass'], d['fail'], d['warn']], start=2):
            c = ws.cell(row=r, column=i, value=v)
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # SHEET 2 — Vulnerability Summary
    # ═══════════════════════════════════════════════════════════════════════
    ws2 = wb.create_sheet("Vulnerability Summary")
    ws2.sheet_view.showGridLines = False
    _perf_xlsx_header_row(ws2, 1, ["Severity", "Count"])
    ws2.freeze_panes = "A2"

    SEV_COLORS = {'critical':'DC2626','high':'EF4444','medium':'F59E0B','low':'10B981','informational':'3B82F6'}
    ...
    for t in tests:
        ...
        label = {'critical':'Critical','high':'High','medium':'Medium','low':'Low','informational':'Informational','info':'Informational'}.get(sev, 'Medium')
        sev_counts[label] += 1

    for i, sev in enumerate(SEV_ORDER, start=2):
        color = SEV_COLORS[sev.lower()]
        c1 = ws2.cell(row=i, column=1, value=sev)
        c1.font = Font(name=_XLSX_FONT, bold=True, size=10, color=color)
        c1.border = _XLSX_BORDER
        c2 = ws2.cell(row=i, column=2, value=sev_counts[sev])
        c2.font = Font(name=_XLSX_FONT, bold=True, size=10, color=color)
        c2.alignment = Alignment(horizontal="center")
        c2.border = _XLSX_BORDER
        if sev_counts[sev] > 0:
            c1.fill = PatternFill("solid", fgColor="F8FAFC")
            c2.fill = PatternFill("solid", fgColor="F8FAFC")
    _perf_xlsx_title(ws2, "VULNERABILITY SUMMARY", "EF4444")
    _perf_xlsx_autofit(ws2, [30, 16])

    # ═══════════════════════════════════════════════════════════════════════
    # SHEET 3 — Security Test Scenarios
    # ═══════════════════════════════════════════════════════════════════════
    ws3 = wb.create_sheet("Test Scenarios")
    ws3.sheet_view.showGridLines = False
    _perf_xlsx_header_row(ws3, 1, ["#", "Test Scenario", "Category", "Severity", "Detail"])
    ws3.freeze_panes = "A2"

    for i, t in enumerate(tests, start=2):
        cat = (t.get('category') or 'auth').lower()
        sev = (t.get('severity') or t.get('priority') or 'medium').lower()
        vals = [i-1, t.get('name',''), cat.upper(), sev.upper(), (t.get('suite') or 'Security check')[:120]]
        for col, v in enumerate(vals, start=1):
            c = ws3.cell(row=i, column=col, value=v)
            c.font = Font(name=_XLSX_FONT, size=9, bold=(col in (3,4)),
                          color=(CAT_COLORS.get(cat,"64748B") if col==3 else SEV_COLORS.get(sev,"F59E0B") if col==4 else "1E293B"))
            c.alignment = Alignment(horizontal="center" if col in (1,3,4) else "left", vertical="top", wrap_text=(col==5))
            c.border = _XLSX_BORDER
    _perf_xlsx_title(ws3, "SECURITY TEST SCENARIOS", "EF4444")
    _perf_xlsx_autofit(ws3, [5, 40, 16, 12, 60])
    ...
    ws4.sheet_view.showGridLines = False
    _perf_xlsx_header_row(ws4, 1, ["Category", "Total", "Passed", "Failed", "Warn", "Status"])
    ws4.freeze_panes = "A2"
    ...
    for cat, d in cats.items():
        verdict = 'PASS' if d['fail'] == 0 else 'FAIL'
        bg = "D1FAE5" if d['fail'] == 0 else "FEE2E2"
        vals = [cat.upper(), d['total'], d['pass'], d['fail'], d['warn'], verdict]
        for col, v in enumerate(vals, start=1):
            c = ws4.cell(row=row, column=col, value=v)
            c.font = Font(name=_XLSX_FONT, size=9, bold=(col in (1,6)),
                          color=("EF4444" if (col==6 and verdict=="FAIL") else "10B981" if (col==6 and verdict=="PASS") else "1E293B"))
            c.fill = PatternFill("solid", fgColor=bg)
            c.alignment = Alignment(horizontal="center" if col > 1 else "left", vertical="center")
            c.border = _XLSX_BORDER
        row += 1
    _perf_xlsx_title(ws4, "RESULTS BY CATEGORY", "EF4444")
    _perf_xlsx_autofit(ws4, [22, 10, 10, 10, 10, 12])
    ...
    ws5.sheet_view.showGridLines = False
    _perf_xlsx_header_row(ws5, 1, ["#", "Test Name", "Category", "Severity", "Status", "Result / Reason", "Duration"])
    ws5.freeze_panes = "A2"

    STATUS_COLOR = {"pass": ("10B981", "D1FAE5"), "fail": ("EF4444", "FEE2E2"), "warn": ("F59E0B", "FFFBEB")}
    for i, t in enumerate(tests, start=2):
        status = t.get('status', 'warn')
        s_color, s_bg = STATUS_COLOR.get(status, ("94A3B8", "FFFFFF"))
        cat = (t.get('category') or 'auth').lower()
        sev = (t.get('severity') or t.get('priority') or 'medium').lower()
        reason = t.get('suite') or t.get('reason') or '—'
        vals = [i-1, t.get('name',''), cat.upper(), sev.upper(), status.upper(), str(reason)[:150], t.get('duration','—')]
        for col, v in enumerate(vals, start=1):
            c = ws5.cell(row=i, column=col, value=v)
            c.font = Font(name=_XLSX_FONT, size=9, bold=(col in (3,4,5)),
                          color=(CAT_COLORS.get(cat,"64748B") if col==3 else SEV_COLORS.get(sev,"F59E0B") if col==4 else s_color if col==5 else "1E293B"))
            c.alignment = Alignment(horizontal="center" if col in (1,3,4,5,7) else "left", vertical="top", wrap_text=(col==6))
            c.border = _XLSX_BORDER
            if col == 5:
                c.fill = PatternFill("solid", fgColor=s_bg)
        ws5.row_dimensions[i].height = 20
    _perf_xlsx_title(ws5, "DETAILED SECURITY RESULTS", "0D9488")
    _perf_xlsx_autofit(ws5, [5, 40, 16, 12, 12, 55, 12])
    ...
    ws6.sheet_view.showGridLines = False
    _perf_xlsx_header_row(ws6, 1, ["Priority", "Category", "Issue", "Fix"])
    ...
    if not recs:
        ...
        c = ws6.cell(row=2, column=1, value="No specific issues flagged by AI for this run.")
        c.font = Font(name=_XLSX_FONT, italic=True, size=9, color="94A3B8")
    _perf_xlsx_title(ws6, "AI RECOMMENDATIONS", "4F46E5")
    _perf_xlsx_autofit(ws6, [12, 18, 40, 55])
    ...
    action_plan = _call_groq_for_plan(tests, url) or []
    PRI_ORDER = {'HIGH':0,'MEDIUM':1,'LOW':2}
    r = 2
    for i, item in enumerate(action_plan, start=1):
        ...
        for col, v in enumerate(vals, start=1):
            ...
            c.font = Font(name=_XLSX_FONT, size=9, bold=(col==4),
                          color=PRI_COLOR.get(priority.lower(), "F59E0B") if col==4 else "1E293B")
            c.alignment = Alignment(vertical="top", wrap_text=(col in (2,5)),
                                     horizontal="center" if col in (1,4,6,7,8) else "left")
            if col == 4:
                c.fill = PatternFill("solid", fgColor=PRI_BG.get(priority.lower(), "FFFBEB"))
            c.border = _XLSX_BORDER
    ...
    for lbl, val in env_items:
        c1 = ws8.cell(row=r, column=1, value=lbl)
        c1.font = Font(name=_XLSX_FONT, bold=True, size=9, color="EF4444")
        c1.fill = PatternFill("solid", fgColor="F8FAFC")
        c1.border = _XLSX_BORDER
        c2 = ws8.cell(row=r, column=2, value=val)
        c2.font = Font(name=_XLSX_FONT, size=9, color="1E293B")
        c2.border = _XLSX_BORDER
        r += 1
    _perf_xlsx_title(ws8, "ENVIRONMENT & EXECUTION INFO", "EF4444")
    _perf_xlsx_autofit(ws8, [24, 40])
    ...
    ws9.sheet_view.showGridLines = False
    _perf_xlsx_header_row(ws9, 1, ["Property", "Value"])
    ws9.freeze_panes = "A2"

    if critical_fail:
        verdict_text = (f'Security Test FAILED — critical vulnerabilities detected on {url} (Auth/XSS/Session). '
                         f'These issues must be fixed before production deployment.')
        verdict_color = "EF4444"
        risk_final = "HIGH"
    elif fail_count > 0 or warn_count > 0:
        verdict_text = (f'Security Test passed with {fail_count} failed / {warn_count} warned check(s) on {url}. '
                         f'Review before deployment.')
        verdict_color = "F59E0B"
        risk_final = "MEDIUM"
    else:
        verdict_text = f'Security Test PASSED — all {pass_count} security checks passed on {url}.'
        verdict_color = "10B981"
        risk_final = "LOW"

    ws9.merge_cells("A1:B1")
    c = ws9.cell(row=1, column=1, value="Final Security Verdict")
    c.font = Font(name=_XLSX_FONT, bold=True, size=13, color=verdict_color)
    r = 2
    r = _add_wrapped_text(ws9, r, verdict_text, Font(name=_XLSX_FONT, size=9, color=verdict_color),
                           fill=("D1FAE5" if risk_final=="LOW" else "FFFBEB" if risk_final=="MEDIUM" else "FEE2E2"),
                           end_col=2)
    ws9.merge_cells(start_row=r, start_column=1, end_row=r, end_column=2)
    c = ws9.cell(row=r, column=1,
                  value=f"Security Score: {score}/100  |  Risk Level: {risk_final}  |  Grade: {grade}")
    c.font = Font(name=_XLSX_FONT, bold=True, size=10, color="1E293B")
    r += 2
    ...
    c = ws9.cell(row=r, column=1, value=f"{url}  —  {score}/100  ({score_label})")
    c.font = Font(name=_XLSX_FONT, bold=True, size=12, color=score_color_hex.lstrip('#').upper())
    c.alignment = Alignment(horizontal="center")
    r += 2
    ws9.merge_cells(f"A{r}:B{r}")
    ...
    _perf_xlsx_title(ws9, "CERTIFICATE OF SECURITY VALIDATION", "EF4444")
    _perf_xlsx_autofit(ws9, [26, 40])

    # ── Ordre final ──────────────────────────────────────────────────────
    order = ["Overview", "Vulnerability Summary", "Test Scenarios", "Results by Category",
